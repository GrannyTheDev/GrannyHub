local EmbeddedModules = {
    ...
    --[[
		...
		local function main()
			...
			local huiNode = {Obj = Instance.new("Folder")}
			local idCounter = 0
			...
			addObject = function(root)
				...
				elseif huiMap[rootParObj] or par == huiNode then
					huiMap[root] = true
					huiCons[root] = huiCons[root] or {
						connectSignal(root.ChildAdded,addObject),
						connectSignal(root.AncestryChanged,moveObject),
					}
					isHui = true
			...
			moveObject = function(obj)
				...
				elseif huiMap[newPar.Obj] or newPar == huiNode then
					huiMap[obj] = true
					huiCons[obj] = huiCons[obj] or {
						connectSignal(obj.ChildAdded,addObject),
						connectSignal(obj.AncestryChanged,moveObject),
			...
			Explorer.InitRightClick = function()
				...
				context:Register("REFRESH_HUI",{Name = "Refresh GetHui", OnClick = function()
					Explorer.RefreshGetHui()
				end})

				context:Register("HIDE_HUI",{Name = "Hide GetHui", OnClick = function()
					Explorer.HideNilInstances()
				end})
			...
			Explorer.HideGetHui = function()
				table.clear(huiMap)

				local disconnectCon = Instance.new("Folder").ChildAdded:Connect(function() end).Disconnect
				for i,v in next,huiCons do
					disconnectCon(v[1])
					disconnectCon(v[2])
				end
				table.clear(huiCons)
				...
				for i = 1,#huiNode do
					coroutine.wrap(removeObject)(huiNode[i].Obj)
				end
			...
			Explorer.RefreshGetHui = function()
				if not env.gethui then return end

				local nilInsts = env.gethui():GetChildren()
				local game = game
				local getDescs = game.GetDescendants
				--local newNilMap = {}
				--local newNilRoots = {}
