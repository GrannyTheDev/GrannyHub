...
local filename = "DevilHub/GrowingSimulator - 12080891934/Config.json"
...
makefolder("DevilHub");
makefolder("DevilHub/GrowingSimulator - 12080891934");
...
if getgenv().Settings.autogrow == true then
if debounce then
    return
end
debounce = true
wait(0.1)
game.ReplicatedStorage.GrowEvent:FireServer()
debounce = false
end
end)
end)
...
local autogrow = AutoFarm:Toggle("Auto Grow", function(v)
getgenv().Settings.autogrow = v
Save()
if v then
doAutoGrow()
end
...
local autorebirth = AutoFarm:Toggle("Auto Rebirth", function(v)
getgenv().Settings.autorebirth = v
Save()
if v then
doAutoRebirth()
end
...
if getgenv().Settings.autogrow == true then
autogrow:ChangeState(true)
end
if getgenv().Settings.autorebirth == true then
autorebirth:ChangeState(true)
end
