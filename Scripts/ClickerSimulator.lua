...
local Window = Library:CreateWindow("GrannyHub".." - Clicker Simulator")
...
local filename = "DevilHub/ClickerSimulator - 7560156054/Config.json"
...
makefolder("DevilHub");
makefolder("DevilHub/ClickerSimulator - 7560156054");
...
for i,v in pairs(getconnections(game:GetService("Players").LocalPlayer.PlayerGui.mainUI.clickerButton.MouseButton1Click)) do
    v.Function()
end
wait(0.1)
end
...
game:GetService("ReplicatedStorage").Events.ClientToServer.ClientToServerEvent:FireServer("LocalScript", "RequestRebirth", getgenv().Settings.rebcount, false, false)
wait(0.1)
end
end)
end

local click = AutoFarm:Toggle("AutoClick", function(v)
...
local rebcount = AutoFarm:Box("Rebirth Amount", "Input rebirth amount", function(v)
getgenv().Settings.rebcount = v
Save()
end)
...
rebcount:ChangeText(getgenv().Settings.rebcount)
