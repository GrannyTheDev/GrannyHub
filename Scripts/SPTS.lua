...
local Window = Library:CreateWindow("GrannyHub".." - SPTS")
...
local filename = "DevilHub/SPTS - 11312500614/Config.json"
...
makefolder("DevilHub");
makefolder("DevilHub/SPTS - 11312500614");
...
AutoFarm:Toggle("Auto Train BT", function(v)
    getgenv().bt = v
    while true do
    if not getgenv().bt then return end
    wait(0.1)
    for i,v in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
        if v:IsA("Tool") and v.Parent:WaitForChild("BTTool", 0.1) then
            v:Activate()
            end
        end
    end
...
AutoFarm:Toggle("Auto Train FS", function(v)
getgenv().fs = v
while true do
if not getgenv().fs then return end
wait(0.1)
for i,v in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
    if v:IsA("Tool") and v.Parent:WaitForChild("FSTool", 0.1) then
        v:Activate()
        end
    end
end
